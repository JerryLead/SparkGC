...
if [ $# -ne 7 ]; then
  echo "Usage: ./rjoin-G1.sh <corePerExecutor> <memPerExecutor> <data_percentage> <table1_hdfs_file> <table2_hdfs_file> <output_hdfs_dir> <logFile>"
  exit 1
fi

/root/spark/spark-2.1.4.19-bin-2.7.1/bin/spark-submit --name "RDDJoin-G1-$1-$2-$3" \
