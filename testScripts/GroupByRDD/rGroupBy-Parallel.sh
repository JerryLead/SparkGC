...
if [ $# -ne 6 ]; then
  echo "Usage: ./rGroupBy-Parallel.sh <corePerExecutor> <memPerExecutor> <data_percentage> <table_hdfs_file> <output_hdfs_dir> <logFile>"
  exit 1
fi

/root/spark/spark-2.1.4.19-bin-2.7.1/bin/spark-submit --name "GroupByRDD-Parallel-$1-$2-$3" \
