...
if [ $# -ne 7 ]; then
  echo "Usage: ./SVM-CMS.sh <corePerExecutor> <memPerExecutor> <data_percentage> <input_file> <iter> <dimension> <logFile>"
  exit 1
fi

/root/spark/spark-2.1.4.19-bin-2.7.1/bin/spark-submit --name "SVM-CMS-$1-$2-$3" \
