...
if [ $# -ne 7 ]; then
  echo "Usage: ./PageRank-G1.sh <corePerExecutor> <memPerExecutor> <data_percentage> <table_hdfs_file> <iter> <output_hdfs_dir> <logFile>"
  exit 1
fi

/root/spark/spark-2.1.4.19-bin-2.7.1/bin/spark-submit --name "PageRank-G1-$1-$2-$3" \
